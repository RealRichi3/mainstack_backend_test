...
router
    .post(
        "/login",
        routerSchemaValidator(RouteValidatorSchema.Auth.login),
        AuthController.login
    )
    .post(
        "/signup",
        routerSchemaValidator(RouteValidatorSchema.Auth.signup),
        AuthController.signup
    )
    .post(
        "/reset-password",
        routerSchemaValidator(RouteValidatorSchema.Auth.forgotPassword),
        AuthController.forgotPassword
    )
    .post(
        "/logout",
        verifyAuth(AuthTokenType.Access),
        AuthenticatedController(AuthController.logout)
    )
    .post(
        "/refresh",
